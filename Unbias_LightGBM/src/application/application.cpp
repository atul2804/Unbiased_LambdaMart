...
void Application::LoadData() {
  ...
  if (config_.boosting_config.is_provide_training_metric) { /// 加载train_metric
...
void Application::InitTrain() { /// 训练初始化
  ...
  // load training data
  LoadData(); /// 加载数据和metric
  // initialize the objective function
  objective_fun_->Init(train_data_->metadata(), train_data_->num_data()); /// 初始化目标函数
  // initialize the boosting
  boosting_->Init(&config_.boosting_config, train_data_.get(), objective_fun_.get(),
                  Common::ConstPtrInVectorWrapper<Metric>(train_metric_)); /// 初始化boosting GBDT
